...
/*   Updated: 2019/06/17 00:54:04 by czhang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static int	draw_pixel(int x, int y, int color, t_mlx r)
{
	ft_putendl("+!");
	ft_putendl(r.mlx_ptr);
	ft_putendl(r.win_ptr);
	if (!color)
		color = 200;
	return (mlx_pixel_put(r.mlx_ptr, r.win_ptr, x, y, color));
}
